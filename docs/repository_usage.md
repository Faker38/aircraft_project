...
```text
信号预处理 -> 数据集管理 -> 生成数据集版本 -> 模型训练数据检查
```

- 预处理页输出候选样本后，会同步到数据集管理页。
- 数据集管理页负责手动标注、映射表自动标注、纳入/排除和数据集版本生成。
- 生成数据集版本后，会输出 `data/datasets/{version_id}/manifest.json`。
- 训练页选择对应版本后，可执行数据检查，查看标签分布、样本文件缺失和划分数量。
...
4. 切到“模型训练”页，选择版本并点击“数据检查”。
5. 简要说明删除样本、删除版本和清空样本数据库按钮用于联调清理，不建议演示时实际清空。

## 待实现 / 协作项

- 预处理算法侧继续优化候选段筛选阈值和真实无人机信号检出效果。
- 训练模块后续接入真实训练调度和正式评估结果。
- 识别模块后续接入真实模型加载与推理服务。
