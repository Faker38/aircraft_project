...
class USRPDemoPreprocessWorker(QObject):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.failed.emit(f"IQ 预处理执行失败：{exc}")
            return

        if not isinstance(result, USRPDemoPreprocessResult):
            self.failed.emit("IQ 预处理失败：服务层未返回有效结果。")
            return
